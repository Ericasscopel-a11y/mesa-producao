import { Home, LayoutGrid, Calendar, BarChart2, Plus } from "lucide-react";
import { C } from "../theme";

export default function BottomNav({ screen, setScreen, setShowAdd }) {
  const tabs = [
    { id: "home", icon: <Home size={22} />, label: "Início" },
    { id: "content", icon: <LayoutGrid size={22} />, label: "Conteúdos" },
    { id: "calendar", icon: <Calendar size={22} />, label: "Calendário" },
    { id: "analytics", icon: <BarChart2 size={22} />, label: "Análises" },
  ];
  return (
    <div style={{ background: C.card, borderTop: `0.5px solid ${C.border}`, display: "flex", paddingBottom: 8 }}>
      {[tabs[0], tabs[1], "plus", tabs[2], tabs[3]].map((n) => {
        if (n === "plus") return (
        ...
        return (
          <button key={n.id} onClick={() => setScreen(n.id)} style={{
            ...
            <span style={{ fontSize: 10, color: active ? C.dark : C.muted, fontWeight: active ? 600 : 400 }}>{n.label}</span>
          </button>
        );
      })}
    </div>
